...
- AV pass 的 prompt 明令**不转写对白**，其产出是证据层；屏显文字与字幕逐字重复时 merge 会告警
  （硬字幕片源防护）。
- AV pass **不给人物命名**，只给描述性称呼（如"红衣女子"）；它输出的称呼与演员表之间唯一的合法桥梁，
  是人签核时绑定的 `visual_label` 字段。
- **命名权在人**（v0.6）：`resolve_cast.py` 只收敛候选、永不产出名字；`cast_signoff.py` 写盘前必须解析出
  人的明确回答，含糊/新名字一律回到人确认；`splice_screenplay.py` 对表外专名说话人**致命**（退出码 9）。
  演员表里每个名字都带 `approved_by: "human"`，机器无法伪造这条记录（有测试钉住）。
- **外发面没有增加**：P0 的声学属性（性别/年龄带/音线）与 P3 的 `mouth_state` 都问在**已付费的同一次**
  Omni 调用里，不新增请求、不新增主机、不上传任何新数据类别。
- **演员表是本地文件**（`<系列根>/cast.approved.json` + 工作区 `.cache/` 快照）：脚本不联网抓取任何
  外部 cast 资料，人工触发的联网校正也必须"agent 检索 → 人确认 → 才写盘"；表里**不存凭据**。
...
| resolve_cast 不能产出没有人工签核记录的名字 | 测试钉住（正例 + 反例成对） |
| 呼语不得作为「这句话是谁说的」的正证据 | 测试钉住（`role` 枚举里无 attribution） |
| 单族证据不得自动合并簇到已签核槽位（§4.1） | 测试钉住 |
| 视觉族在遵从未达标时不参与判定（不是反证） | 测试钉住 |
| 演员表只写进用户 `--bind` 显式指定的系列目录 | 仅约定：该路径在工作区之外，不做逃逸校验（与工作区内的 `output/` 检查不同）；`resolve_cast.py` 写 `cast.json` 仍受工作区包含校验 |
