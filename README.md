...
3. **Dual-Track Extraction** — FFmpeg shot cuts + keyframes run concurrently with subtitle ingestion and **acoustic speaker diarization** (ffmpeg extracts 16 kHz mono audio → MCP `omni_multi_speaker_asr` clusters voices by timbre → deterministic max-overlap binding to subtitle lines). Multi-part runs snap cut points to silence, overlap adjacent parts by ±3 s, and reconcile cross-part identities only on co-occurrence evidence (the same utterance diarized in both parts — union-find, prefer splitting over wrong merging). Attribution is 100% acoustic; subtitle metadata only votes to NAME the clusters. Without the MCP tool, speakers degrade to all-null and the pipeline keeps running.
